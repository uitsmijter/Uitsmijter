...
- ❌ No actual `/.well-known/jwks.json` endpoint exists
- ❌ Uses HS256 (symmetric) instead of RS256 (asymmetric) for JWT signing
- ❌ Cannot expose public keys (HS256 has no public key)
- ⚠️ Advertises RS256 support but doesn't implement it
...
**Assessment**: ❌ Uses symmetric HS256, needs to be replaced with RS256
...
**Assessment**: ✅ Already advertises JWKS URI, but endpoint doesn't exist
...
**Assessment**: ⚠️ Advertises RS256 but actually uses HS256 (false advertising!)
...
**Assessment**: ✅ Standard OIDC payload structure, compatible with RS256
...
**Acceptance Criteria**:
- [x] Can generate 2048-bit RSA key pairs
- [ ] Can generate 4096-bit RSA key pairs
- [x] Generates unique `kid` based on timestamp
- [x] Extracts public key components (n, e) correctly
- [x] Returns both signer and JWK representation
- [x] Thread-safe (actor isolation)
...
**Test Scenarios**:
1. Generate RSA key → Sign JWT → Verify with JWKS public key
2. Key rotation: Multiple keys → Verify old and new tokens
3. Migration: Sign with RS256 → Verify old HS256 tokens
4. External verification: Export JWKS → Verify with external library
...
**Test Scenarios**:
1. Fetch /.well-known/openid-configuration → Extract jwks_uri → Fetch JWKS
2. Obtain OAuth token → Extract kid from header → Match kid in JWKS
...
**Content**:
- Overview of HS256 → RS256 migration
...
### Phase 1: RSA Key Management ⏳ PENDING
- [ ] Task 1.1: Create RSA Key Pair Generator
- [ ] Task 1.2: Create JWK Data Models
- [ ] Task 1.3: Implement Key Storage
- [ ] Task 1.4: Update Signer to Support RS256

### Phase 2: JWKS Endpoint Implementation ⏳ PENDING
- [ ] Task 2.1: Create JWKS Controller
- [ ] Task 2.2: Register JWKS Route
- [ ] Task 2.3: Update JWT Payload to Include "kid"
...
### Phase 6: CHANGELOG and Release ⏳ PENDING
- [ ] Task 6.1: Update CHANGELOG
- [ ] Task 6.2: Create Release Notes
...
### High Risk
- ⚠️ **Breaking Change**: Migrating from HS256 to RS256 could break existing integrations
- ⚠️ **Key Management**: RSA key generation and storage must be secure
- ⚠️ **Performance**: RSA operations are slower than HMAC

### Medium Risk
- ⚠️ **Migration Complexity**: Dual-mode support adds code complexity
- ⚠️ **Key Rotation**: Coordination needed during key rollover
- ⚠️ **Backward Compatibility**: Must support old HS256 tokens during migration

### Low Risk
- ✅ **JWKS Endpoint**: Straightforward implementation, well-defined spec
- ✅ **JWTKit Support**: Library already supports RS256
...
1. ✅ RFC 7517 compliance validated
2. ✅ `/.well-known/jwks.json` endpoint returns valid JWK Set
3. ✅ RS256 signing and verification works correctly
4. ✅ Key rotation supported with multiple active keys
5. ✅ Backward compatibility maintained during migration
6. ✅ All unit tests pass (>90% coverage)
7. ✅ Integration tests pass
8. ✅ E2E tests pass in all browsers
9. ✅ Documentation complete and accurate
10. ✅ No security vulnerabilities introduced
11. ✅ Performance acceptable (token signing/verification < 10ms)
12. ✅ Metrics tracking implemented
