...
function kindSetupCert() {
  ...
  openssl req -x509 -newkey ec \
    ...
    -addext 'subjectAltName=DNS:*.localhost,DNS:example.com,DNS:*.example.com,DNS:ham.test,DNS:*.ham.test,DNS:*.s3.ham.test,DNS:bnbc.example,DNS:*.bnbc.example,*.egg.example.com'
}
