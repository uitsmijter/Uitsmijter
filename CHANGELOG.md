...
- Feature: **Predefined Providers (`uitrusting/v1`)** - A tenant's `providers` entry may now be a predefined-provider object (`{ type: "uitrusting/v1", url, token? }`) instead of a hand-written script. Uitsmijter generates the `UserLoginProvider` and `UserValidationProvider` internally; they call the external service's `POST {url}/verify` (login sends `{ tenant, namespace, username, password_hash }`, refresh re-validation sends the same without the hash) and map the `{ known, valid, subject, roles, scopes, profile }` response onto the provider interface. The plain password is never transmitted — only its SHA256 hash. An optional `token` is sent as the `X-Internal-Token` header for internal-only endpoints.
- Feature: **Tenant Context for JavaScript Providers** - Provider scripts now receive the tenant alongside the credentials as `credentials.tenant` (`{ name, namespace }`), so a provider can scope its request to the correct tenant (e.g. against a multi-tenant user service). Raw script providers that ignore the field are unaffected.
- Feature: **Roles Claim** - JWT tokens may now carry an optional `roles` array claim in addition to the single `role` (which mirrors the primary role). Providers can expose a `roles` getter; the claim is omitted for single-role providers, so existing tokens are unchanged.

- Fix: **client_secret is no longer required on `/authorize`** - Per RFC 6749 §4.1.1 / §3.2.1 the authorization endpoint only identifies the client by `client_id`; confidential clients authenticate with their secret on the back-channel token request. Requiring it on `/authorize` broke spec-conformant OIDC clients and leaked the secret through the user-agent. The secret is now validated only at the token endpoint.
- Fix: **RFC 8628-Compliant Device Token Errors** - The device token endpoint returns standard OAuth error responses (`HTTP 400` with `{"error":"authorization_pending"|"slow_down"|"access_denied"|"invalid_grant"}`) instead of the generic Uitsmijter error envelope, and `slow_down` is a `400` rather than `429`. It also accepts the standard `urn:ietf:params:oauth:grant-type:device_code` grant-type URN (keeping the bare `device_code` as an alias), so standard OAuth2 client libraries interoperate with the device flow.
- Fix: **Traefik 3.x** - Additional fixes for running behind Traefik 3.x.
- Fix: **Reproducible e2e** - Pinned the Traefik Helm chart used by the end-to-end suite (its newer values schema rejected the tracked values), and ignore browser crash core dumps left in the Playwright test directory.

- Change: **`device_grant_config` is optional** - The device grant is enabled solely by listing `device_code` in a client's `grant_types`. When `device_grant_config` is omitted, sensible defaults apply (`expires_in: 1800`, `interval: 5`, auto-detected verification URI) instead of rejecting the request.
- Change: **Tenant `providers` Schema** - Each `providers` entry may now be either a raw JavaScript script (string, as before) or a predefined-provider object. The Tenant CRD schema accepts both shapes.
- Change: **soto updated to 7.x** - The AWS SDK dependency (used for S3 tenant-template loading) was bumped from 6.x to 7.x (soto-core 7, async-http-client 1.30+), and the S3 template loader was migrated to the soto 7 API.

- Improvement: **Device Activation Success Page** - The "device authorized successfully" page now has a styled, centered layout with a success checkmark instead of plain left-aligned text.
- Improvement: **DeviceGrantDemo CLI** - A small Rust reference client (built on the standard `oauth2` crate) that runs the full device flow against a Uitsmijter server, under `Tests/Tools/Cli/DeviceGrantDemo`. It doubles as a conformance check for the RFC-compliant grant.
